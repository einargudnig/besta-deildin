...
export class InsufficientBudgetError extends Error { 
  constructor(availableBudget: number, requiredBudget: number) {
    super(`Insufficient budget: Available budget is ${availableBudget}, required budget is ${requiredBudget}`);
...
export const fantasyTeamService = {
  async addPlayerToTeam(
    ...
    isOnBench: boolean
  ): Promise<Result<SelectedPlayer, InsufficientBudgetError | PositionLimitError | TeamLimitError>> {
    try {
